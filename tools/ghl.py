...
def _get_ghl_social_accounts(location_id: str) -> list:
    ...
    try:
        ...
        if "authorized for this scope" in msg.lower():
            raise RuntimeError(
                "GHL API key is missing Social Planner scope for this location. "
                "Update Private Integration permissions for social-media-posting, "
                "or set GHL_SOCIAL_ACCOUNT_IDS to bypass account discovery."
            ) from exc
        raise RuntimeError(f"GHL social account discovery failed: {msg}") from exc
    except Exception as exc:
        raise RuntimeError(f"GHL social account discovery failed: {exc}") from exc
...
def publish_content_to_ghl_social(content_item: dict) -> dict:
    ...
    # Optional media generated by Creative Director lane.
    media_urls = []
    inline_media = content_item.get("media_url") or content_item.get("image_url")
    if inline_media:
        media_urls.append(str(inline_media).strip())
    listed_media = content_item.get("media_urls") or []
    if isinstance(listed_media, list):
