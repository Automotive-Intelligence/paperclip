...
def sofia_generate_video(
    ...
    """Generate a short-form video clip via the hybrid video lane.

    Two backends:
      - 'kling' (default): Replicate Kling — cheap, fast B-roll for the
        weekly content cadence on IG Reels / TikTok / FB short.
      - 'seedance_pro': fal.ai ByteDance Seedance Pro — premium cinematic
        B-roll for P&P beauty shots, brand video, and Book'd ad B-roll
        where motion quality matters. Costs ~$0.62 per 5s clip vs ~$0.05
        on Kling, so reserve it for premium use.

    HeyGen talking-head lipsync is NOT this tool — that's a separate lane.

    Optionally pass start_image_url to do image-to-video (e.g., animate
    a journal cover shot generated by sofia_generate_image).
    ...
    Args:
        ...
        model: 'kling' (default, cheap) or 'seedance_pro' (premium, fal.ai).
    ...
    # Pre-flight: clean error string for missing fal.ai key, so the LLM gets
    # actionable feedback instead of an opaque "RuntimeError".
