...
def test_config_has_seven_sites_and_brands():
    ...
    # WD re-enabled 2026-08-01 (now publishes via Slipstream, human-merged ~weekly)
    assert cfg["brands"]["worship_digital"]["blog_max_age_hours"] == 192
    # still-disabled gap brands stay 0
    assert cfg["brands"]["paper_and_purpose"]["blog_max_age_hours"] == 0
